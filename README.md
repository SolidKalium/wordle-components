...
- Analysis HTML UI
  - Brute force list: fix count
    ...
    - This probably belongs on the constraint itself.
  - Constraint editor: fix non-normalization of values. The inputs and output were supposed to be separate, with the output being a normalized constraint. Maybe we need to distinguish a "game constraint" from a "raw constraint", at least in terms of input, but they should produce the same output. And that includes an example like __ate with t not in either of the first two spots causing the t to promoted to "gray" automatically and removed from "not at". But the constraint viewer doesn't show that normalization. It's possible that the current error messages should be generated by the constraint itself instead of the editor.
